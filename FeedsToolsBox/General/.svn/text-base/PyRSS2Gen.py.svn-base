...
def _element(handler, name, obj, d = {}):
    ...
    elif hasattr(obj,"publish"):
        # It better know how to emit the correct XML.
        obj.publish(handler)
    else:
        handler.startElement(name, d)
        handler.characters(str(obj))
        handler.endElement(name)
...
class Enclosure:
    ...
    def __init__(self, url=None, length=None, type=None):
        self.url = url
        self.length = length
        self.type = type
    def publish(self, handler):
        enclosureDict={}
        if self.url != None:
            enclosureDict["url"]=self.url
        if self.length != None:
            enclosureDict["length"]=self.length
        if self.url != None:
            enclosureDict["type"]=self.type
        _element(handler, "enclosure", None,enclosureDict)
